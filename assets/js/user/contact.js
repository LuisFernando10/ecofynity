    ...
    $('.js-contact-btn-send-email').on('click', function () {
        ...
        if (value_name === '' || value_name.length === 0){
            notify_error_notification('El <b>Nombre</b> es obligatorio.', 2000);
            control_validation = false;
            return false;
        }
        else if (value_email === '' || value_email.length === 0){
            notify_error_notification('El <b>Correo Electrónico</b> es obligatorio.', 2000);
            control_validation = false;
            return false;
        }
        else if ((value_email !== '' || value_email.length > 0) && validateEmail(value_email) === false){
            notify_error_notification('El <b>Correo Electrónico</b> es inválido.', 2000);
            control_validation = false;
            return false;
        }
        else if (value_phone.length > 0 && value_phone.length <= 5){
            notify_error_notification('El <b>Teléfono</b> debe ser mayor a 5 dígitos.', 2000);
            control_validation = false;
            return false;
        }
        else if ((value_phone.length > 5 && value_phone.length < 32) && validateNumber(value_phone) === false){
            notify_error_notification('El <b>Teléfono</b> debe contener sólo números.', 2000);
            control_validation = false;
            return false;
        }
        else if (value_text === '' || value_text.length === 0){
            notify_error_notification('El <b>Mensaje</b> es obligatorio.', 2000);
            control_validation = false;
            return false;
        }

        //Validamos si todo etá Ok
